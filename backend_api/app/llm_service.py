...
try:
    client = anthropic.Anthropic() # API key is picked up from environment automatically
...
# Define the model name we'll use (Claude 3 Haiku is good for speed/cost)
DEFAULT_LLM_MODEL = "claude-3-haiku-20240307"

class LLMArticleOutput(BaseModel):
    summary: str
    table_of_contents: List[str] # List of heading titles
    article_sections: List[Dict[str, Any]] # e.g., [{"heading": "Intro", "content": "...", "timestamp_start": 0.0}, ...]
...
async def generate_article_from_transcript(
...
) -> LLMArticleOutput: # Changed return type to non-Optional as we always return an object
    """
    ...
    if not ANTHROPIC_API_KEY_AVAILABLE or client is None:
        ...
        return LLMArticleOutput(
            ...
            table_of_contents=[],
            article_sections=[{"heading": "Error", "content": "LLM processing failed due to configuration.", "timestamp_start": 0.0, "relevant_to_search_intent": False}]
        )

    if not transcript_segments: # Check if list is empty
        print("LLM Service: Transcript segments list is empty, cannot generate article.")
        return LLMArticleOutput(
            summary="Transcript was empty.",
            table_of_contents=[],
            article_sections=[{"heading": "Error", "content": "Transcript was empty.", "timestamp_start": 0.0, "relevant_to_search_intent": False}]
        )
    
    full_transcript_text = " ".join([segment.get('text', '') for segment in transcript_segments]) # Use .get for safety
    if not full_transcript_text.strip():
        ...
        return LLMArticleOutput(
            ...
            table_of_contents=[],
            article_sections=[{"heading": "Error", "content": "Transcript text was empty.", "timestamp_start": 0.0, "relevant_to_search_intent": False}]
        )

    system_prompt = """You are an expert summarizer and article writer. Your task is to transform a raw video transcript into a well-structured, readable article.
...
3. The main article content, as a list of objects, where each object represents a section and has the following keys:
   ...
   - "relevant_to_search_intent": (boolean) True if this section is particularly relevant to the user's search intent, false otherwise.

Ensure the article flows well and reads like a coherent piece, not just a reformatted transcript. Rephrase spoken language into more formal written language where appropriate.
...
"""
    ...
    # Limiting segments to keep prompt reasonable for now and avoid excessive costs during dev
    # For very long transcripts, a summarization/chunking strategy will be needed eventually.
    for segment in transcript_segments[:150]: # Reduced limit slightly for initial broad testing
    ...
    if user_search_intent:
        user_message_content += f"\nThe user's search intent is: \"{user_search_intent}\". Please identify sections relevant to this intent."
    ...
    print("\n--- Sending to LLM ---")
    # For debugging, you might want to print parts of the user_message_content if issues persist
    # print(f"User Message Snippet: {user_message_content[:1000]}") # Print first 1000 chars
    
    try:
        response = client.messages.create(
            model=model_name,
            max_tokens=4096, # Max for Haiku output tokens
        ...
        if response.content and isinstance(response.content, list) and len(response.content) > 0 and hasattr(response.content[0], 'text'):
            ...
            try:
                ...
                # Robust handling for 'summary'
                raw_summary = parsed_json.get("summary") # Get can return None if key is missing
                final_summary = "Summary not provided or in unexpected format." # Default error
                if isinstance(raw_summary, str):
                    final_summary = raw_summary
                elif isinstance(raw_summary, list):
                    if len(raw_summary) > 0 and isinstance(raw_summary[0], str):
                        final_summary = raw_summary[0] # Take the first string
                ...
                # Robust handling for 'table_of_contents'
                raw_toc = parsed_json.get("table_of_contents")
                final_toc = []
                if isinstance(raw_toc, list):
                    final_toc = [str(item) for item in raw_toc if isinstance(item, str)] # Ensure all items are strings
                ...
                # Robust handling for 'article_sections'
                ...
                if isinstance(raw_sections, list):
                    for section in raw_sections:
                        if isinstance(section, dict):
                            ...
                            heading = section.get("heading", "Missing Heading")
                            content = section.get("content", "Missing Content")
                            relevant = section.get("relevant_to_search_intent", False) # Default to False
                            
                            if not isinstance(heading, str): heading = str(heading) # Coerce to string
                            if not isinstance(content, str): content = str(content) # Coerce to string
                            if not isinstance(relevant, bool): relevant = False # Default if not boolean

                            final_sections.append({
                                "heading": heading,
                                "content": content,
                                "relevant_to_search_intent": relevant
                                # Consider adding placeholder for timestamp_start if needed by Pydantic model
                                # and LLM isn't providing it directly in this sub-object.
                                # For now, LLMArticleOutput.article_sections is List[Dict[str, Any]]
                                # so it's flexible.
                            })
                        else:
                            print(f"Warning: An item in article_sections was not a dictionary, got {type(section).__name__}. Skipping.")
